                        ...
                        <table id="add-row" class="display table table-striped table-hover text-center">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ optional($data->dokterUgd)->nama }}</td>
                                    <td>
                                        @foreach($data->dokterRawatInap as $dokter)
                                        {{ $dokter->nama }}<br>
                                        @endforeach
                                    </td>
                                    <td>
                                        ...
                                        <!-- Tombol Hapus -->
                                        <form id="delete-form-{{ $data->id }}" action="{{ route('rawat-inap.delete', $data->id) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('rawat-inap.edit', $data->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                <label for="dokter_ugd">Dokter UGD</label>
                                <select name="dokter_ugd" class="form-control" required>
                                    @foreach($dokters as $dokter)
                                    <option value="{{ $dokter->id }}" {{ $data->dokter_ugd_id == $dokter->id ? 'selected' : '' }}>{{ $dokter->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="dokter_rawat_inap">Dokter Rawat Inap</label>
                                <select name="dokter_rawat_inap[]" class="form-control" multiple required>
                                    @foreach($dokters as $dokter)
                                    <option value="{{ $dokter->id }}" {{ in_array($dokter->id, $data->dokterRawatInap->pluck('id')->toArray()) ? 'selected' : '' }}>{{ $dokter->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
